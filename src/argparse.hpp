...
class BasicArgument {
 ...
 public:
  ...
  class Exception : public std::exception {
   ...
   public:
    ...
    Exception(std::string msg = "") { _msg = std::string(msg); }
 ...
 protected:
  ...
  bool type_assert() {
    switch (_base_type) {
      case BaseType::TYPE_BOOL:
        return std::is_same<bool, T>::value;
      case BaseType::TYPE_CHAR:
        return std::is_same<char, T>::value;
      case BaseType::TYPE_INT:
        return std::is_same<int, T>::value;
      case BaseType::TYPE_LONG_LONG:
        return std::is_same<long long, T>::value;
      case BaseType::TYPE_DOUBLE:
        return std::is_same<double, T>::value;
      case BaseType::TYPE_STRING:
  ...
  void init() {
    ...
    if (_base_type == BaseType::TYPE_ERROR)
      throw std::runtime_error("init(): unsupported type " +
...
class Argument : public BasicArgument {
 ...
 public:
  ...
  Argument& choices(std::vector<T> choice_values) {
    ...
    _choices.clear();
    for (auto& it : choice_values) _choices.insert(it);
  ...
  Argument& nArgs(const char ch) {
    if (std::is_same<bool, T>::value && ch == '+')
      throw BasicArgument::Exception(this,
                                     "nArgs(): not available on bool args");
    ...
    if (ch == '*')
      ...
      throw BasicArgument::Exception(
          this, "nArgs(): unknown tag '" + std::to_string(ch) + "'");
 ...
 protected:
    ...
    try {
      switch (_base_type) {
        ...
        case BaseType::TYPE_CHAR:
          return str.length() == 1;
        case BaseType::TYPE_INT: {
          volatile int t = std::stoi(str);
          break;
        }
        case BaseType::TYPE_LONG_LONG: {
          volatile long long t = std::stoll(str);
          break;
        }
        case BaseType::TYPE_DOUBLE: {
          volatile double t = std::stod(str);
          break;
        }
        case BaseType::TYPE_STRING:
          break;
  ...
  void add_value(std::string str) override {
    ...
    if (!value_assert(str_))
      throw BasicArgument::Exception(
          this, "add_value(): mistyped value '" + str_ + "'");
...
class ArgumentParser {
  ...
  std::unordered_map<std::string, int>
      _tag_map;  // mapping short string to pos of argument-structure
  std::unordered_map<std::string, int>
      _name_map;  // mapping long string to pos of argument-structure
  ...
  BasicArgument* _get_argument(std::string index) {
    ...
    int pos =
        p == 1
            ? (_tag_map.find(name) == _tag_map.end() ? -1 : _tag_map[name])
 ...
 public:
  ...
  Argument<T>& add_argument(std::string tag, std::string name = "") {
    ...
    if (tag_type[0] == 0) {
      ...
      if (!_positional_flag)
        throw BasicArgument::Exception(
            "add_argument(): positional args should be set before optional "
            "args");
      _positional_num++;
    ...
    if (name.length() > 0) _name_map[name_f] = _args.size() - 1;
  ...
  T get_value(const char* index) {
    ...
    if (iter == _tag_map.end() || iter == _name_map.end())
      throw BasicArgument::Exception("get_value(): argument not found: " +
                                     std::string(index));
    ...
    if (!arg->type_assert<T>())
      throw BasicArgument::Exception(
          arg,
          "get_value(): invalid cast <" + std::string(typeid(T).name()) + ">");
    ...
    for (auto it : _args) {
      ...
      if (it->call_()) {
        ...
        for (auto it2 = clist.begin(); it2 != clist.end(); it2++) {
          if (is_call(*it2))
            throw BasicArgument::Exception(
                it, "conflicts(): conflict against '" + *it2 + "'");
        }
      }
    }
  }
  ...
  void show_help() {
    ...
    std::cout << "\n\npositional arguments:\n";
    for (int i = 0; i < _positional_num; i++)
      std::cout << "  " << _args[i]->tag() << "\t" << _args[i]->help()
                << std::endl;
